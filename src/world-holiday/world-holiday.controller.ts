...
export class WorldHolidayController {
  ...
  @ApiOperation({
    description: 'Get supported states e.g. for US.',
  ...
  @ApiOperation({
    description: 'Get supported regions e.g. for US, Lousiana.',
  ...
  public async getRegion(@Param() param, @Res() res) {
    ...
    const countryName = param.countryName;
    const result = await this.worldHolidayService.getRegions(
      countryName,
      stateName,
    );
    res.status(result.status).json(result.msg);
  ...
  public async getCountryHolidays(@Param() param, @Query() query, @Res() res) {
    const countryName = param.countryName;
